...
public class withdrawPureEss extends Task {
    private Player me;
    ...
    {
        me = Players.getLocal();
        return atBankArea.contains(me)&& Equipment.contains("Binding necklace") && Equipment.containsAnyOf(anyRing) && !Inventory.contains("Pure essence");
    }
    ...
    public void execute()
    {
        if(Bank.isOpen())
        {
            Bank.withdraw("Pure essence",100);
            Execution.delayUntil(()->Inventory.contains("Pure essence"),1000,2000);
            getLogger().debug("Withdrew pure essence");
        }
